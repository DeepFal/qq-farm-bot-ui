...
const SHOP_PLANTING_STRATEGY_LIST = ['preferred', 'level', 'max_exp', 'max_fert_exp', 'max_profit', 'max_fert_profit'];
const SHOP_PLANTING_STRATEGY_SET = new Set(SHOP_PLANTING_STRATEGY_LIST);
...
async function plantFromShop(landsToPlant, state, strategyOverride = '') {
    ...
    let planted = 0;
    try {
        const result = await plantSeeds(actualSeedId, targetLandIds, { maxPlantCount: needCount });
        planted = result.planted;
        plantedLandIds = uniqPositiveNumbers(result.plantedLandIds);
        occupiedLandIds = uniqPositiveNumbers(result.occupiedLandIds);
        ...
        log('种植', plantSize > 1
            ...
            : `已在 ${planted} 块地种植 (${occupiedLandIds.join(',') || targetLandIds.slice(0, planted).join(',')})`, {
            module: 'farm',
            event: 'plant_seed',
            result: 'ok',
            seedId: actualSeedId,
...
async function autoPlantEmptyLands(deadLandIds, emptyLandIds) {
    const landsToPlant = uniqPositiveNumbers(emptyLandIds);
    const state = getUserState();
    const plantedLandIds = [];
    let totalPlanted = 0;
    let occupiedCount = 0;
    ...
    if (deadLandIds.length > 0) {
        try {
            ...
            landsToPlant.push(...uniqPositiveNumbers(deadLandIds));
        } catch (e) {
    ...
    if (opType === 'all' || opType === 'plant') {
        ...
        if (allDeadLands.length > 0 || allEmptyLands.length > 0) {
            try {
                ...
                const plantedCount = Number(plantResult && plantResult.occupiedCount) || 0;
                if (plantedCount > 0) {
